...
def loadcolumn(filename,col=4, skip=1, floats=True):
    pred=[]
    op=open(filename,'r')            #X_test=np.column_stack((X_test,np.loadtxt("histogram_features_testclean.csv", delimiter=",") ))
            #X=np.column_stack((X,np.loadtxt("histogram_features_trainclean.csv", delimiter=",") ))
    if skip==1:
        op.readline() #header
    for line in op:
        line=line.replace('\n','')
...
def main():
        ...
        outset="marios_nnnew_v4" # predic of all files
