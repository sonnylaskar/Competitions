...
def find_null_columns(x_array):
    list_of_null_cols=[]
    for k in range(0,x_array.shape[1]):
        if len(np.unique(x_array[:,k]))==1:
            print("appending: " , str(k))
            list_of_null_cols.append(k)
    print(list_of_null_cols)
    print("done")
...
def main():
        ...
        outset="marios_xg_v1" #prefix

        #model to use

        model=xg.XGBoostClassifier(num_round=1000 ,nthread=threads,  eta=0.02, gamma=7.0,max_depth=20, min_child_weight=1, subsample=0.9,
                                   colsample_bytree=0.55,objective='binary:logistic',seed=1)
