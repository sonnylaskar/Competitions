...
def main():
        ...
        model=xg.XGBoostClassifier(num_round=1000 ,nthread=threads,  eta=0.02, gamma=7.0,max_depth=15, min_child_weight=20, subsample=0.9,
                                   colsample_bytree=0.4,objective='rank:pairwise',seed=1)

        if not os.path.exists(metafolder):      #if it does not exists, we create it
            os.makedirs(metafolder)

        outset="marios_xgrank_v3" # predic of all files
